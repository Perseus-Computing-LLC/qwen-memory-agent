# Devpost Submission — Perseus Vault MemoryAgent

## Project Name
Perseus Vault MemoryAgent — Persistent AI Memory on Qwen Cloud

## Elevator Pitch (200 chars)
Perseus Vault gives AI agents persistent, searchable memory across sessions — remembering facts, forgetting what's stale, and surfacing context that makes agents smarter over time. Powered by Qwen Cloud.

## Track
Track 1: MemoryAgent

## What it does
Perseus Vault MemoryAgent solves the fundamental problem of AI amnesia. Every LLM conversation starts from zero — no memory of past projects, preferences, decisions, or corrections. Perseus Vault fixes this by wrapping a production-grade persistent memory backend (27 MCP tools, FTS5+vector hybrid search, AES-256-GCM encryption, Ebbinghaus decay) with Qwen Cloud reasoning models.
...
## How we built it
- **Memory backend**: Perseus Vault (Rust) — production persistent memory with SQLite, FTS5, vector search, and Ebbinghaus decay
...
- Perseus Vault's MCP stdio protocol requires persistent process management — used direct CLI subprocess calls for hackathon simplicity
...
- Not a toy — Perseus Vault is a real memory system with 27 MCP tools, used in production agents
